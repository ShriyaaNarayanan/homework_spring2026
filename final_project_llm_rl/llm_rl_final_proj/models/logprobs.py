...
def compute_per_token_logprobs(
    ...
    *,
    ...
    with torch.set_grad_enabled(enable_grad):
        ...
        logits = out.logits
        B, L, V = logits.shape
        target = input_ids[:, 1:].reshape(-1) # shape [B * (L-1)]
        logprobs = F.cross_entropy(logits[:, :-1, :].reshape(-1, V), target, reduction="none")
        return logprobs.reshape(B, L-1)
